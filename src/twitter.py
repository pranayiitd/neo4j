...
def get_followers(uid,type,version,client):
	...
	# Loop to get all the pages of response not more than 3 req/user (15k relations)
...
# Make call to twitter API version to get all followers of user with uid
def get_friends(uid,type,version,client):
	...
	while(limit_remaining > 3 and cursor!='0'):
		...
		if (count==3):
			break
		api_call = "https://api.twitter.com/"+str(version)+"/friends/ids.json?cursor="+cursor+"&user_id="+str(uid)
		if(type==1):
			api_call = "https://api.twitter.com/"+str(version)+"/friends/ids.json?cursor="+cursor+"&screen_name="+str(uid)
